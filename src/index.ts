...
export class Bodyguard {
	...
	constructor(
		...
		this.config = {
			...
			maxDepth: config.maxDepth && typeof config.maxDepth === "number" && config.maxDepth > 0
				? config.maxDepth
				: MAX_DEPTH,
			maxSize: config.maxSize && typeof config.maxSize === "number" && config.maxSize > 0 ? config.maxSize : MAX_SIZE,
			maxKeyLength: config.maxKeyLength && typeof config.maxKeyLength === "number" && config.maxKeyLength > 0
				? config.maxKeyLength
				: MAX_KEY_LENGTH,
			castBooleans: config.castBooleans !== undefined && typeof config.castBooleans === "boolean"
				? config.castBooleans
				: false,
			castNumbers: config.castNumbers !== undefined && typeof config.castNumbers === "boolean"
				? config.castNumbers
				: false,
		};
	}
	...
	>(
		...
		const parser = bodyType === "params"
			? new URLParamsParser(instanceConfig)
			: new FormDataParser(instanceConfig, boundary);
	...
	private constructConfig(config?: Partial<BodyguardConfig>): BodyguardConfig {
		return {
			maxKeys: config?.maxKeys && typeof config.maxKeys === "number" && config.maxKeys > 0
				? config.maxKeys
				: this.config.maxKeys,
			maxDepth: config?.maxDepth && typeof config.maxDepth === "number" && config.maxDepth > 0
				? config.maxDepth
				: this.config.maxDepth,
			maxSize: config?.maxSize && typeof config.maxSize === "number" && config.maxSize > 0
				? config.maxSize
				: this.config.maxSize,
			maxKeyLength: config?.maxKeyLength && typeof config.maxKeyLength === "number" && config.maxKeyLength > 0
				? config.maxKeyLength
